...
class ProxyRotator:
    ...
    def __init__(self, storage_dir: str = 'proxy'):
    ...
    def refresh_proxies(self) -> List[Dict[str, Any]]:
        ...
        self.valid_proxies = self.storage.get_valid_proxies()
        self.last_refresh = current_time
        
        # Reset failed proxies if enough time has passed
        if current_time - getattr(self, 'last_failure_reset', 0) > self.failure_reset_time:
        ...
        logger.info(f"Refreshed proxy list: {len(available_proxies)} available proxies "
...
class HTTPProxyServer:
    ...
    def __init__(self, host: str = '127.0.0.1', port: int = 8888, 
                 storage_dir: str = 'proxy', rotation_mode: str = 'round-robin'):
        self.host = host
        self.port = port
        self.rotator = ProxyRotator(storage_dir)
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='HTTP Proxy Server with verified proxy rotation')
    parser.add_argument('--host', default='127.0.0.1', help='Server host (default: 127.0.0.1)')
    parser.add_argument('--port', type=int, default=8888, help='Server port (default: 8888)')
    parser.add_argument('--storage', default='proxy', help='Proxy storage directory (default: proxy)')
