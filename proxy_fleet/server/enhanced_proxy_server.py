...
class GracefulShutdownManager:
    ...
    async def add_request(self, task: asyncio.Task):
        ...
        async with self._lock:
            ...
            # 當任務完成時自動清理
            task.add_done_callback(lambda t: asyncio.create_task(self._remove_request(t)))
    ...
    async def start_shutdown(self):
        ...
        if active_count > 0:
            ...
            try:
                ...
                logger.warning(f"⚠️  Shutdown timeout reached, {remaining} requests still active")
        else:
...
class ProxyStats:
    ...
    def update_circuit_breaker(self, success: bool, config: Dict[str, Any]):
        ...
        if (self.circuit_breaker_state == CircuitBreakerState.OPEN and
            current_time - self.circuit_breaker_last_failure >= recovery_timeout):
    ...
    def is_available(self) -> bool:
        ...
        return (self.is_healthy and 
                self.circuit_breaker_state in [CircuitBreakerState.CLOSED, CircuitBreakerState.HALF_OPEN])


class EnhancedProxyRotator:
    """Advanced proxy rotation with multiple load balancing strategies"""
    
    def __init__(self, storage_dir: str = 'proxy', config: Dict[str, Any] = None):
        self.storage = ProxyStorage(storage_dir)
        self.config = config or {}
        self.lb_config = self.config.get('load_balancing', {})
        self.strategy = LoadBalancingStrategy(self.lb_config.get('strategy', 'round_robin'))
        ...
        self.proxy_weights = self.lb_config.get('strategies', {}).get('weighted', {}).get('proxy_weights', {})
    ...
    async def refresh_proxies(self) -> List[Dict[str, Any]]:
        ...
        with self.lock:
            ...
            all_proxies = self.storage.get_valid_proxies()
            ...
            self.proxy_stats = {k: v for k, v in self.proxy_stats.items() if k in valid_keys}
            ...
            logger.info(f"Refreshed proxy list: {len(self.available_proxies)} available "
    ...
    def _get_round_robin_proxy(self, proxies: List[Dict[str, Any]]) -> Tuple[Dict[str, Any], ProxyStats]:
    ...
    def _get_random_proxy(self, proxies: List[Dict[str, Any]]) -> Tuple[Dict[str, Any], ProxyStats]:
        """Random selection"""
        proxy = random.choice(proxies)
        stats = self.proxy_stats[f"{proxy['host']}:{proxy['port']}"]
        return proxy, stats

    def _get_least_connections_proxy(self, proxies: List[Dict[str, Any]]) -> Tuple[Dict[str, Any], ProxyStats]:
        """Least connections selection"""
        best_proxy = min(proxies, key=lambda p: self.proxy_stats[f"{p['host']}:{p['port']}"].active_connections)
        stats = self.proxy_stats[f"{best_proxy['host']}:{best_proxy['port']}"]
        return best_proxy, stats

    def _get_weighted_proxy(self, proxies: List[Dict[str, Any]]) -> Tuple[Dict[str, Any], ProxyStats]:
    ...
    def _get_fastest_proxy(self, proxies: List[Dict[str, Any]]) -> Tuple[Dict[str, Any], ProxyStats]:
        """Response time based selection"""
        best_proxy = min(proxies, key=lambda p: self.proxy_stats[f"{p['host']}:{p['port']}"].average_response_time)
        stats = self.proxy_stats[f"{best_proxy['host']}:{best_proxy['port']}"]
        return best_proxy, stats

    def _get_failover_proxy(self, proxies: List[Dict[str, Any]]) -> Tuple[Dict[str, Any], ProxyStats]:
    ...
    async def record_request_result(self, proxy_host: str, proxy_port: int, 
            ...
            if proxy_key in self.proxy_stats:
                ...
                if not success:
                    if stats.consecutive_failures >= unhealthy_threshold:
                        ...
                        logger.warning(f"Marked proxy {proxy_key} as unhealthy after {stats.consecutive_failures} failures")
                else:
                    if stats.consecutive_successes >= healthy_threshold and not stats.is_healthy:
                        stats.is_healthy = True
                        logger.info(f"Marked proxy {proxy_key} as healthy after {stats.consecutive_successes} successes")
    ...
    def decrement_connections(self, proxy_host: str, proxy_port: int):
        ...
        with self.lock:
            if proxy_key in self.proxy_stats:
                self.proxy_stats[proxy_key].active_connections = max(0, 
                    self.proxy_stats[proxy_key].active_connections - 1)
    ...
    async def _perform_health_checks(self):
        ...
        async def check_proxy(proxy):
            ...
            try:
                ...
                if proxy.get('protocol', 'socks5') in ['socks4', 'socks5']:
                    proxy_type = ProxyType.SOCKS5 if proxy['protocol'] == 'socks5' else ProxyType.SOCKS4
                    connector = ProxyConnector(proxy_type=proxy_type, host=proxy['host'], port=proxy['port'])
                    session_kwargs = {'connector': connector}
                else:
                    connector = aiohttp.TCPConnector()
                    session_kwargs = {'connector': connector, 'proxy': f"http://{proxy['host']}:{proxy['port']}"}
                
                client_timeout = ClientTimeout(total=timeout)
                async with ClientSession(timeout=client_timeout, **session_kwargs) as session:
                    async with session.get(test_url) as response:
                        if response.status == 200:
                            response_time = time.time() - start_time
                            await self.record_request_result(proxy['host'], proxy['port'], True, response_time)
                            logger.debug(f"Health check OK for {proxy_key} ({response_time:.2f}s)")
                        else:
                            await self.record_request_result(proxy['host'], proxy['port'], False)
                            logger.warning(f"Health check failed for {proxy_key}: HTTP {response.status}")
    ...
    def get_stats_summary(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            total_failed = sum(stats.failed_requests for stats in self.proxy_stats.values())
...
class EnhancedHTTPProxyServer:
    ...
    def __init__(self, config_file: str = None):
        ...
        # Initialize rotator
        self.rotator = EnhancedProxyRotator(config=self.config)
    ...
    async def handle_request(self, request: Request) -> Response:
        ...
        try:
            ...
            if proxy_protocol in ['socks4', 'socks5']:
                ...
                connector = ProxyConnector(proxy_type=proxy_type, host=proxy_host, port=proxy_port)
                session_kwargs = {'connector': connector}
            else:  # http proxy
                connector = aiohttp.TCPConnector()
                session_kwargs = {'connector': connector, 'proxy': f"http://{proxy_host}:{proxy_port}"}
            ...
            async with ClientSession(timeout=timeout, **session_kwargs) as session:
                async with session.request(
                    ...
                    # Record success
                    await self.rotator.record_request_result(proxy_host, proxy_port, True, response_time)
                    self.stats['requests_success'] += 1
                    
                    logger.info(f"✅ {method} {url} -> {proxy_host}:{proxy_port} "
    ...
    async def start_worker(self):
        ...
        try:
            ...
            logger.info(f"🛑 Shutting down worker {os.getpid()}...")
        finally:
    ...
    def start_multiprocess(self):
        ...
        logger.info(f"🔄 Load balancing strategy: {self.config['load_balancing']['strategy']}")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced HTTP Proxy Server with load balancing')
    parser.add_argument('--config', default='proxy_server_config.json', 
